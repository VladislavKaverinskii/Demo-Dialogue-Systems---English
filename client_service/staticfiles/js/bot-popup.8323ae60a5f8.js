$(document).ready(function(){
	...
	$.ajax({
				url: "http://178.128.245.158:9090/get_history/",
				data: {},
				type: "post",
	...
	function restoreDialog(data) {		
		if (data.length > 0) {
			console.log(data);
			$(".msg-insert").empty();					
			$('.msg-insert').append("<div class='msg-receive'>Hello! This is a reference system for SDG. What interests you? </div>");
			...
			$(".chat-text_input").attr("placeholder", "Enter your question...");
		} else {
	...
	textarea.keypress(function(event) {
		...
		if(event.keyCode == 13){
			...
			$.ajax({
				url: "http://178.128.245.158:9090/process_question/",
				data: current_data,
				type: "post",
	...
	function requestAnswer(data) {
		...
		if ((request_counter <= 3000) &&(typeof data["tech_response"] != "undefined") && (data["tech_response"] == "is_standard")) {
			...
			$.ajax({
				url: "http://178.128.245.158:9090/get_answer/",
	...
	$(".msg-insert").on('click', ".stop_dialog-button-positive", function(e){
		...
		$(".start-dialog_replic").remove();
		$.ajax({
			url: "http://178.128.245.158:9090/ask_unsubscribe/",
			data: {"result": "True"},
			type: "post",
